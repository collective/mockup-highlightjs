...
# Make tasks
all: test-once bundle-highlightjs
...
bundle-highlightjs:
	# Build highlightjs bundle
	mkdir -p build
	NODE_PATH=$(NODE_PATH) $(GRUNT) bundle-highlightjs $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-highlightjs/js/Gruntfile.js
...
ifeq ($(NODE_VERSION_LT_011),true)
	...
	NODE_PATH=$(NODE_PATH) $(BOWER) install --config.interactive=0 -g
	NODE_PATH=$(NODE_PATH) $(GRUNT) sed:bootstrap $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-highlightjs/js/Gruntfile.js

jshint:
	# Codecheck
	NODE_PATH=$(NODE_PATH) $(GRUNT) jshint $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-highlightjs/js/Gruntfile.js

watch:
	# Run grunt and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) watch $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-highlightjs/js/Gruntfile.js

test:
	# Run tests headless via PhantomJS and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=src/mockup-highlightjs/js/Gruntfile.js

test-once:
	# Run the tests headless with PhantomJS only once and exit.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test_once $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=src/mockup-highlightjs/js/Gruntfile.js

test-dev:
	# Run the tests for Chromium and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test_dev $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=src/mockup-highlightjs/js/Gruntfile.js
...
.PHONY: all plone5 bundle-highlightjs bootstrap jshint watch test test-once test-dev clean clean-deep
